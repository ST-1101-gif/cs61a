...
def pick(paragraphs, select, k):
    ...
    # BEGIN PROBLEM 1
    "*** YOUR CODE HERE ***"
    selected = [paras for paras in paragraphs if select(paras)]
    if len(selected) <= k:
        return ''
    else:
        return selected[k]
...
def accuracy(typed, source):
    ...
    for i in range(min(len(typed_words), len(source_words))):
        if typed_words[i] == source_words[i]:
            cor_num += 1
    return cor_num / len(typed_words) * 100
    # END PROBLEM 3
...
def memo_diff(diff_function):
    ...
    def memoized(typed, source, limit):
        ...
        key = (typed, source)
        if key in cache:
            for cached_limit, result in sorted(cache[key].items()):
...
def time_per_word(words, timestamps_per_player):
    ...
    for player in range(len(tpp)):
        time = [tpp[player][i] - tpp[player][i - 1] for i in range(1, len(tpp[player]))]
        times.append(time)
    # END PROBLEM 9
    ...
    for word_index in range(len(words)):
        ...
        for player in range(len(times)):    
            ...
            if time < min_time:
                min_time = time
                f_player = player
        f_words[f_player].append(words[word_index])
    return f_words
    # END PROBLEM 10
