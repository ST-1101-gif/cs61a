...
class Ant(Insect):
    ...
    def add_to(self, place):
        if place.ant is None:
            ...
            # BEGIN Problem 8b
            if place.ant.is_container and self.is_container or not place.ant.is_container and not self.is_container:
...
class ThrowerAnt(Ant):
    ...
    def nearest_bee(self):
        ...
        step = 0
        for step in range(1, self.lower_bound + 1):
...
class QueenAnt(ThrowerAnt):
    ...
    def action(self, gamestate):
        ...
        behind_plc = self.place.exit
        while behind_plc:
            if behind_plc.ant:
                if not behind_plc.ant.damage_doubled:
                    behind_plc.ant.damage_doubled = True
                    behind_plc.ant.damage *= 2
                if behind_plc.ant.is_container and behind_plc.ant.ant_contained:
                    if not behind_plc.ant.ant_contained.damage_doubled:
                        behind_plc.ant.ant_contained.damage_doubled = True
                        behind_plc.ant.ant_contained.damage *= 2
            behind_plc = behind_plc.exit
        # END Problem 12
...
class LaserAnt(ThrowerAnt):
    ...
    def insects_in_front(self):
        ...
        fron_plc = self.place
        dist = 0
        while not fron_plc.is_hive:
            if fron_plc.ant and fron_plc.ant is not self:
                insects[fron_plc.ant] = dist
                if fron_plc.ant.is_container and fron_plc.ant.ant_contained is not self:
                    insects[fron_plc.ant.ant_contained] = dist
            
            for b in fron_plc.bees:
                insects[b] = dist

            dist += 1
            fron_plc = fron_plc.entrance
...
class Bee(Insect):
    ...
    def scare(self, length):
        ...
        # BEGIN Problem EC 2
        "*** YOUR CODE HERE ***"
        if not self.have_scared:
            self.back_step = length
